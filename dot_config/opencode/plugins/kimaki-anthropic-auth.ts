...
type AuthLoaderGetAuth = () => Promise<OAuthStored | { type: string }>;
  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`Timed out acquiring OAuth refresh lock: ${lockPath}`);
      }
      await sleep(150);
    }
  }
...
async function withAuthRefreshLock<T>(fn: () => Promise<T>) {
  ...
  const release = await acquireLock(authLockPath());
  try {
    return await fn();
  } finally {
    await release();
...
async function createApiKey(accessToken: string): Promise<ApiKeySuccess> {
  ...
  if (!json.raw_key) throw new Error("Anthropic API key creation returned no raw_key");
...
function buildAuthorizeHandler(mode: "oauth" | "apikey") {
  return async () => {
    ...
    const finalize = async (result: CallbackResult): Promise<AuthResult> => {
      ...
      if (mode === "apikey") return createApiKey(creds.access);
      return creds;
    };
        ...
        callback: async (): Promise<AuthResult> => {
          pendingAuthResult ??= (async () => {
            try {
              ...
              console.error(`[kimaki-anthropic-auth] ${error}`);
              return { type: "failed" };
            }
          })();
...
function wrapResponseStream(response: Response, reverseToolNameMap: Map<string, string>) {
  ...
  const transform = (text: string) =>
    text.replace(/"name"\s*:\s*"([^"]+)"/g, (full, name: string) => {
...
async function getFreshOAuth(getAuth: AuthLoaderGetAuth, client: PluginClient) {
...
export const kimakiAnthropicAuthPlugin = async ({ client }: PluginInput) => {
  return {
    auth: {
      provider: "anthropic",
      async loader(
        getAuth: AuthLoaderGetAuth,
