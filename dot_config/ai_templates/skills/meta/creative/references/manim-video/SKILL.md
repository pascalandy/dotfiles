...
Run `scripts/setup.sh` to verify all dependencies. Requires: `uv` (https://docs.astral.sh/uv/), Manim Community Edition v0.20+ (`uv tool install manim`), LaTeX (`texlive-full` on Linux, `mactex` on macOS), and ffmpeg. Reference docs tested against Manim CE v0.20.1.
