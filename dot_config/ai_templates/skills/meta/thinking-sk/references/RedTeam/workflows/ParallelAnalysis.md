...
### Launch Protocol
If multi-agent execution is available, deploy up to 32 specialists in parallel. Otherwise, run smaller batches or sequential passes that still cover engineers, architects, pentesters, and interns. Each perspective receives:
...
- Multi-perspective execution, ideally parallel when available
- Synthesis capability to process multiple perspective outputs
...
- Phase 2: 3-5 minutes with parallel execution, longer if run sequentially
