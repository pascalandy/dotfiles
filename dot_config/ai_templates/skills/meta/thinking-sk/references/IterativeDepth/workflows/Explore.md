...
Read `../references/TheLenses.md` for the lens definitions being used this run.
...
- **Standard (4 lenses):** Run lenses 1-2 inline. If parallel execution is available, run lenses 3-4 concurrently; otherwise run them sequentially. Merge results.

- **Deep (8 lenses):** If parallel execution is available, run four pairs of lenses concurrently (or eight individual workers). Otherwise run the lenses sequentially in batches. Each pass gets:
