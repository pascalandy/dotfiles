...
If multi-agent execution is available, run one council member per specialist in parallel. Otherwise, run the same four perspectives sequentially while keeping each voice distinct.

**Each agent prompt includes:**
```
You are [Agent Name], [brief role description from `references/CouncilMembers.md`].
...
If multi-agent execution is available, run Round 2 in parallel. Otherwise, run each response sequentially using the same transcript.
...
If multi-agent execution is available, run Round 3 in parallel. Otherwise, run each synthesis sequentially using the full transcript.
...
| Council Role | Focus | Personality Reference |
|--------------|-------|----------------------|
| Architect | System design, patterns, scalability | Serena Blackwood |
| Designer | UX, accessibility, user impact | Aditi Sharma |
| Engineer | Implementation reality, maintenance | Marcus Webb |
| Researcher | Evidence, precedent, external examples | Ava Chen |
| Security | Attack surface, abuse cases, risk | Rook Blackburn |
| Fresh Eyes | Naive questions, assumption checks | Custom composed |
| Writer | Framing, clarity, communication | Emma Hartley |
