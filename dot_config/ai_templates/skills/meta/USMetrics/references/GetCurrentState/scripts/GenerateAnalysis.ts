...
function calculateTrend(observations: Observation[], years: number): TrendStat | null {
  ...
  if (Math.abs(pctChange) < 2) direction = "stable";
  else if (pctChange > 0) direction = "up";
  else direction = "down";
...
`;
  ...
  if (unrate?.latest) {
    md += `- **Unemployment** at ${formatValue(unrate.latest.value, "UNRATE")} (${directionArrow(unrate.trends["1y"]?.direction || "stable")} YoY)\n`;
  }
  if (cpi?.trends["1y"]) {
    md += `- **Inflation (CPI)** running ${cpi.trends["1y"].pctChange.toFixed(1)}% YoY\n`;
  }
  if (gdp?.trends["1y"]) {
    md += `- **Real GDP** ${directionArrow(gdp.trends["1y"].direction)} ${Math.abs(gdp.trends["1y"].pctChange).toFixed(1)}% over past year\n`;
  }
  if (fedfunds?.latest) {
    ...
    for (const id of cat.ids) {
      ...
      if (r?.latest && r.trends["1y"]) {
        const changeStr = r.trends["1y"].pctChange >= 0
          ...
          : `${r.trends["1y"].pctChange.toFixed(1)}%`;
        md += `| ${cat.name} | ${r.name} | ${formatValue(r.latest.value, id)} | ${changeStr} | ${directionArrow(r.trends["1y"].direction)} |\n`;
      }
    }
  }
...
*Analysis generated by USMetrics skill*
`;
