 ...
 *   US_METRICS_DATA_DIR   - Optional data directory (default: ~/Projects/Substrate/Data/US-Common-Metrics)
 *
 * Output files (in US_METRICS_DATA_DIR):
...
const DATA_DIR = process.env.US_METRICS_DATA_DIR || join(process.env.HOME || "", "Projects/Substrate/Data/US-Common-Metrics");
...
async function fetchAllMetrics(): Promise<Map<string, FetchResult>> {
  ...
  for (const [id, config] of Object.entries(METRICS)) {
    ...
    if (data) {
      ...
      console.log(`    OK ${formatValue(data.value, config)} (${data.date})`);
    } else {
      errors.push(id);
      console.log(`    FAIL`);
    }
...
async function main() {
  ...
  if (values.help) {
    console.log(`
UpdateSubstrateMetrics.ts - Update US-Common-Metrics dataset
...
Environment:
  FRED_API_KEY          Required for most metrics
  EIA_API_KEY           Required for gas prices
  US_METRICS_DATA_DIR   Data directory (default: ~/Projects/Substrate/Data/US-Common-Metrics)
...
`);
  ...
  if (!existsSync(DATA_DIR)) {
    ...
    console.error("Set US_METRICS_DATA_DIR to point to your metrics data directory.");
    process.exit(1);
  }
  ...
  console.log(`  OK ${historicalCsvPath} (appended)`);
