...
# Report - use stat instead of ls for file size
FILE_SIZE=$(stat -f%z "$OUTPUT" 2>/dev/null || stat -c%s "$OUTPUT" 2>/dev/null)
FILE_SIZE_HUMAN=$(echo "$FILE_SIZE" | awk '{ split("B KB MB GB TB PB", unit); u=1; while($1>=1024){$1/=1024; u++} printf "%.1f %s", $1, unit[u] }')
echo ""
echo "=== Done ==="
echo "Output: $OUTPUT ($FILE_SIZE_HUMAN)"
