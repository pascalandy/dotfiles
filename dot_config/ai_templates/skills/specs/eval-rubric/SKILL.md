...
| 0     | Unassessable          | The criterion cannot be judged from the final artifact                                                   | no           |

## Evaluation Logic

For each criterion:

1. Can this criterion be judged from the artifact? If no → `0`.
...
- **Score**: `0`-`4`
- **Decision**: `yes` or `no`
- **Justification**: One sentence citing observable evidence from the artifact.
...
- Never average scores across criteria. The decision is per-criterion: count pass/fail, do not blend.
