---
name: mermaid-og
description: Use when user needs diagrams, visualizations, flowcharts, sequence diagrams, ERDs, class diagrams, C4 architecture, state machines, git graphs, gantt charts, pie charts, or mentions Mermaid. Triggers include "diagram", "visualize", "model", "map out", "show the flow", or documenting system architecture, APIs, database schemas, or processes.
---
...
- First line declares diagram type (`flowchart`, `sequenceDiagram`, `classDiagram`, ...)
- Use `%%` for comments
- Unknown words break diagrams; parameters fail silently — validate in [Mermaid Live](https://mermaid.live)
...
**Also available (inline, no dedicated reference):** state diagrams (`stateDiagram-v2`), git graphs (`gitGraph`), gantt charts (`gantt`), pie charts (`pie`). See [Mermaid docs](https://mermaid.js.org/) for syntax.
...
1. **Start simple** — core entities/components first, add details incrementally
2. **One concept per diagram** — split large diagrams into multiple focused views
3. **Meaningful labels** — self-documenting beats terse-and-clever
4. **Comment with `%%`** — explain non-obvious relationships and styling choices
5. **Version with code** — store `.mmd` files alongside the systems they describe
6. **Ensure contrast** — verify Frappe choices remain readable in target render target
7. **Use ELK for complex layouts** — when dagre produces crossed lines

## When to Diagram

- Starting new projects or features
- Documenting complex systems or architecture decisions
- Designing database schemas
- Planning refactors before coding
- Onboarding new contributors

## Export

- **GitHub/GitLab**: renders automatically in `.md` files
- **VS Code / Obsidian / Notion / Confluence**: native or plugin support
- **Mermaid Live**: https://mermaid.live (PNG/SVG export)
- **CLI**: `npm i -g @mermaid-js/mermaid-cli` then `mmdc -i diagram.mmd -o output.png`
