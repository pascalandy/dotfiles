...
- Model template at `../references/ModelTemplate.md`
- Web research is helpful, but user-provided analysis is sufficient when external research is unavailable
...
Read `WorldModels/INDEX.md` if it exists.
...
Tell the user you are updating world threat models and whether this is a full rebuild or targeted refresh.

### Step 2: Determine Update Scope

**Full Creation** (no models exist or user says "rebuild all"):
- Create all 11 models from scratch
- If parallel execution is available, split horizons across concurrent workers for efficiency. Otherwise run them sequentially.
...
1. **Gather research** (lightweight or extensive depending on scope):
...
3. **Optional concurrency**: When creating multiple models, parallelize by horizon if available. Otherwise process them sequentially in batches:
   - Near-term batch: 6mo, 1yr, 2yr, 3yr
   - Mid-term batch: 5yr, 7yr, 10yr
   - Long-term batch: 15yr, 20yr, 30yr, 50yr

### Step 4: Write Model Documents

For each model, follow `../references/ModelTemplate.md`:
...
Save to: `WorldModels/{horizon}.md`

### Step 5: Update INDEX

Write/update `WorldModels/INDEX.md`:
...
### Step 6: Completion Update

Tell the user how many horizons were refreshed and whether any models still need follow-up research.

## Optional Parallel Prompt Template

If multi-agent execution is available, use this prompt structure for each horizon worker:
...
{INSERT ../references/ModelTemplate.md CONTENT}
...
RESEARCH: Use web search or equivalent research capability to find current data, forecasts, and analysis relevant to
...
| Skill | Purpose |
|-------|---------|
| **Web research** | Primary data gathering for model content and current projections |
