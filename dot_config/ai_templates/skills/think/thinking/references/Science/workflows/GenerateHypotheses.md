...
If multi-agent execution is available, generate ideas from different angles in parallel. Otherwise, run the same perspectives sequentially.
...
For important problems, use multiple perspectives:

```
Perspective 1: "Generate hypotheses from a technical perspective"
Perspective 2: "Generate hypotheses from a user perspective"
Perspective 3: "Generate hypotheses that challenge our assumptions"
Perspective 4: "Generate orthogonal hypotheses from analogous domains"
```

If parallel execution is available, run those perspectives concurrently. Otherwise, work through them one at a time, then merge and deduplicate the results.
