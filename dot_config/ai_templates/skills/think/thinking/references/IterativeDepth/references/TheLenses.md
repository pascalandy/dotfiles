...
At **Standard**, 4 lenses run. If parallel execution is available, these can be run as 2 concurrent pairs; otherwise run them sequentially.

At **Deep**, all 8 lenses run. If parallel execution is available, run them as 4 pairs or 8 individual workers; otherwise run them sequentially for maximum coverage.
