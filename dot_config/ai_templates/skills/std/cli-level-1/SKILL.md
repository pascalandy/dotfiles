...
Apply these requirements to every CLI script or tool being built. They ensure the result is operable by AI agents, composable in pipelines, and safe to retry — while staying usable by humans.
...
When the script has subcommands, the top-level help lists subcommand names with one-line descriptions only. Detailed flags and examples live in each subcommand's own `--help`. This avoids dumping an overwhelming wall of text on the first invocation.
...
If the script has multiple subcommands or resources, pick one pattern and apply it uniformly:

- **noun-verb**: `mycli service list`, `mycli service create`, `mycli config list`
