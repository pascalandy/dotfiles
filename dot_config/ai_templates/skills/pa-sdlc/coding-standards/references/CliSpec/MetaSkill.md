---
name: CliSpec
description: Design CLI parameters and UX (arguments, flags, subcommands, help text, output formats, error messages, exit codes, config/env precedence). USE WHEN design CLI, CLI spec, CLI parameters, CLI surface area, command tree, USAGE synopsis, args table, flags table, exit code map, config precedence, refactor CLI surface, CLI interface design.
---

# CliSpec -- Design CLI Surface Area

Design CLI syntax and behavior before implementation. Human-first, script-friendly.

---

## Core Concept

Lock the interface contract before writing code. A CLI spec defines the command tree, arguments, flags, output modes, error behavior, and configuration precedence -- everything a user or script interacts with. Getting this right first prevents rework and ensures consistency.
...
Produce a compact spec the user can implement:
...
Return the completed spec template with all sections filled in. Use tables for args/flags and exit codes. Include realistic example invocations that demonstrate common workflows, piping, and error cases.
...
- This skill produces the spec. For implementation, the router directs to CliImpl. For auditing, CliAudit.
