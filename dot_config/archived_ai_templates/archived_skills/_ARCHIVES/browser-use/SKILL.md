...
**Always use `--profile "Default"` for sites that may require authentication** (Reddit, GitHub, Gmail, etc.). This uses your Chrome profile with existing cookies/logins.

```bash
browser-use --profile "Default" open <url>    # RECOMMENDED for most sites
browser-use open <url>                        # Only for public/unauthenticated pages
```

## Core Workflow

1. **Navigate**: `browser-use --profile "Default" open <url>` — starts browser with Chrome profile
...
### Authenticated Browsing (Default Approach)

**Always start with Chrome profile** for any site that might need login:

```bash
browser-use --profile "Default" open https://reddit.com/r/example
```

This creates a new Chrome instance using your Default profile's cookies. No need to log in again.
...
**What doesn't work:**
- `browser-use open <url>` — Fresh headless Chromium, no cookies
- `browser-use --connect` — Requires Chrome launched with `--remote-debugging-port=9222`
- Arc/Brave profiles — Detected by `profile list` but only Chrome profiles are supported
...
1. **Use `--profile "Default"` by default** — Avoids auth blocks on Reddit, GitHub, etc.
2. **Always run `state` first** to see available elements and their indices
3. **Use `--headed` for debugging** to see what the browser is doing
4. **Sessions persist** — browser stays open between commands
5. **CLI aliases**: `bu`, `browser`, and `browseruse` all work

## Troubleshooting

- **Blocked by "network security" / auth wall?** Use `--profile "Default"` to use Chrome cookies
