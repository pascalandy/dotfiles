...
Some harnesses support named agent types (e.g., `superpowers:code-reviewer`). Codex does not have a named agent registry — `spawn_agent` creates generic agents from built-in roles (`default`, `explorer`, `worker`).
...
Gemini CLI does not support subagent dispatch. Skills that rely on subagent dispatch (`subagent-driven-development`, `dispatching-parallel-agents`) fall back to single-session execution via `executing-plans`. No configuration can enable this — it is a platform limitation.

### Additional Gemini CLI tools

These tools are unique to Gemini CLI. Use them when available to improve the experience:
...
| `superpowers:plan` / `superpowers:writing-plans` | `references/writing-plans.md` | Implementation planning methodology |
