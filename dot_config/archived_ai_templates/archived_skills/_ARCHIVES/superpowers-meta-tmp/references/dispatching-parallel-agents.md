...
Parallel dispatch requires the harness to support concurrent subagent execution. Check `harness-compat.md` for your platform's capabilities. On harnesses without subagent support, approximate by sequencing agents one after another with the same isolated-context prompt discipline. The quality of parallel work is significantly higher when true parallel execution is available.
