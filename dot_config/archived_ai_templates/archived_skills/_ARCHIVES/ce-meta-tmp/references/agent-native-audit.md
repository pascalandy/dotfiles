...
When the platform supports sub-agent type hints, use an exploratory sub-agent type (e.g., `Explore`) for each principle audit agent, as these perform broad codebase scanning rather than targeted edits.
