...
> Build web interfaces with genuine design quality, avoiding generic AI aesthetics, across the full lifecycle: detect context, plan, build, and verify visually.

## When to Use

- Building any frontend work: landing pages, web apps, dashboards, admin panels, components, interactive experiences
- Greenfield builds or modifications to existing applications
- When design guidance, composition, typography, color, motion, or copy quality is needed
- When the user wants a UI that avoids "AI slop" aesthetics

## Inputs

- Target project or codebase (may be empty for greenfield)
- Description of what needs to be built
- Any explicit design instructions from the user
...
3. **Skill defaults** — apply in greenfield work or when user asks for design guidance

When working in an existing codebase, follow those patterns. When the user specifies a direction that contradicts a default, follow the user.
...
Search file contents and find files matching these patterns to scan for signals.

**Mode Classification:**

| Signals detected | Mode | Behavior |
|---|---|---|
| 4+ across multiple categories | **Existing system** | Defer entirely. Aesthetic opinions (typography, color, motion) yield to the established system. Structural guidance (composition, copy, accessibility, verification) still applies. |
| 1–3 | **Partial system** | Follow what exists; apply skill defaults only for areas where no convention was detected. |
| None | **Greenfield** | Full skill guidance applies. |
| Contradictory / unclear | **Ambiguous** | Ask the user before proceeding. |

**When context is ambiguous**, prompt the user: "I found [detected signals]. Should I follow your existing design patterns or create something distinctive?" Wait for the answer before proceeding. If no way to block for a question, assume "partial" mode and proceed conservatively.
...
These are opinionated defaults the user can override:
...
**Tool preference cascade** — use the first available option:
...
2. **Browser MCP tools** — if browser automation tools are available in the agent's environment, use them.
3. **agent-browser CLI** — if nothing else is available, this is the default.
...
- Visual thesis, content plan, and interaction plan written before code
- Litmus checks passed
- Visual verification completed (screenshot taken, glaring issues fixed)
- No quality-floor violations (broken contrast, missing focus states, div soup, AI commentary in UI)
- New work matches existing design system when in Module C

## Outputs

- Implemented frontend feature, component, page, or application
- Screenshot from visual verification pass
- (Greenfield) Visual thesis, content plan, and interaction plan stated before implementation

## Feeds Into

- `ce:review` / `ce:review-beta` — code review before PR
- `feature-video` — record a walkthrough of the finished UI
- `qa` / `qa-only` — QA testing after implementation

## Harness Notes

- Layer 0 context detection uses file search and content search tools available in the harness (Glob/Grep in Claude Code, equivalent in others).
- Visual verification uses the first available browser tool in the harness environment. Fall back to mental litmus check review if no browser access is possible.
- Blocking user questions (ambiguous mode, etc.) use the harness's native question tool. If none available, present the question as text and await the user's reply before proceeding.
