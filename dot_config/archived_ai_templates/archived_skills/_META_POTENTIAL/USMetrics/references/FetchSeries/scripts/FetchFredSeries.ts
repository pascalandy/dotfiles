 ...
 *
 * Usage:
 *   bun run FetchFredSeries.ts <series_id> [--years=10]
 *   bun run FetchFredSeries.ts UNRATE --years=10
 *   bun run FetchFredSeries.ts --all --years=10
 *
 * Environment:
...
async function main() {
  ...
  if (values.help) {
    console.log(`
FetchFredSeries.ts - Fetch economic data from FRED API

Usage:
  bun run FetchFredSeries.ts <series_id> [options]
  bun run FetchFredSeries.ts --all [options]
...
Examples:
  bun run FetchFredSeries.ts UNRATE
  bun run FetchFredSeries.ts GDPC1 --years=20 --trends
  bun run FetchFredSeries.ts --all --json > data.json
