...
- Ask questions **one at a time** — use the platform's blocking question tool when available. Otherwise, present numbered options in plain text and wait for the user's reply before continuing
...
Always present choices using the platform's blocking question tool when available. Otherwise, present numbered options in plain text and wait for the user's reply before proceeding.
...
4. If not:
   ...
   b. Draft the smallest addition that communicates the three things. Match the file's existing style and density. The addition should describe the knowledge store itself, not the surrounding skill pack.
