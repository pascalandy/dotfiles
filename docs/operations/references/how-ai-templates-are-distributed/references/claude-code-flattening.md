...
description: How `fct_compile_assets` merges the four source subtrees into one flat skills directory that every agent home receives
tags:
  - area/ea
  - kind/doc
  - status/open
...
The source tree under `dot_config/ai_templates/skills/` is organized into four category subtrees: `meta/`, `pa-sdlc/`, `specs/`, and `utils/`. Every skill lives in exactly one of those four categories. The fan-out script flattens them into a single peer-level directory before handing the result to rsync, so every agent home receives a flat `skills/` tree with all four categories merged into one level. The category structure exists only in the source; it is source-side metadata, not runtime structure.
...
1. **Detection.** It checks for any of the four known category subdirectories. If at least one exists, it treats the input as categorized.
2. **Categorized path.** It loops over the four categories in a fixed order (`meta` → `pa-sdlc` → `specs` → `utils`) and does `cp -r` for each one that exists, copying the *contents* of the category directory into the compile root. `cp` is additive — no `--delete` — so each call adds to the compile directory rather than overwriting it.
3. **Flat path.** If none of the four category directories exist, it treats the input as already flat and just copies everything across. This is how the `commands/` input is handled: commands do not use category subtrees, so the flat branch applies.
4. **Silent collisions.** The `2>/dev/null || true` suffix swallows errors. A cross-subtree filename collision (two categories containing the same skill directory name) is resolved silently by "later write wins", since `cp -r` on macOS will overwrite without complaint when it can and silently fail when it cannot.

The compile function is called twice per apply — once for commands, once for skills — in `fct_sync_agent_assets` at `.chezmoiscripts/run_after_backup.sh:111-114`:
...
After these two calls, `$compiled_skills_dir` contains every skill from every category as a direct child, and `$compiled_commands_dir` contains every command as a direct child.

## The single rsync per agent

With the compile step done, each agent home receives exactly two rsync calls (one for commands, one for skills), both sourcing from the compiled scratch directories. From `.chezmoiscripts/run_after_backup.sh:116-146`:
...
4. **Cleanup is explicit.** `fct_sync_agent_assets` ends with `rm -rf "$compiled_commands_dir" "$compiled_skills_dir"` at `.chezmoiscripts/run_after_backup.sh:149`, removing both scratch directories after the fan-out completes. The render-stage scratch directory is still cleaned up by the `trap` in `fct_main`.

## Cross-subtree name collisions

The compile step has one remaining trap: if two category subtrees contain a skill with the same directory name, the later category (in `meta` → `pa-sdlc` → `specs` → `utils` order) wins, silently. `cp -r` on macOS does not warn on overwrite, and the `|| true` suffix suppresses errors.

Examples that would collide:

| `dot_config/ai_templates/skills/pa-sdlc/foo/` | `dot_config/ai_templates/skills/utils/foo/` | Result in compile dir |
|---|---|---|
| exists | exists | `foo/` = `utils/foo/` contents (later category wins) |
| exists | does not exist | `foo/` = `pa-sdlc/foo/` contents |
| does not exist | exists | `foo/` = `utils/foo/` contents |
...
```bash
ls ~/.local/share/chezmoi/dot_config/ai_templates/skills/meta/ \
   ~/.local/share/chezmoi/dot_config/ai_templates/skills/pa-sdlc/ \
   ~/.local/share/chezmoi/dot_config/ai_templates/skills/specs/ \
   ~/.local/share/chezmoi/dot_config/ai_templates/skills/utils/
```

If the name already exists under any of the four categories, pick a different one. The old advice "every other agent keeps the category split, so you are safe except for Claude Code" no longer applies.
