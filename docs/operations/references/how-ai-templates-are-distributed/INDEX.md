...
- the new or renamed file does not collide with another skill after the Claude Code flattening (`meta/`, `pa-sdlc/`, `specs/`, `utils/` all land in `~/.claude/skills/` as siblings)
...
- [claude-code-flattening.md](references/claude-code-flattening.md) — the `~/.claude/skills/` flattening gotcha and the naming collision risk
...
| `references/claude-code-flattening.md` | Why the four-subtree flatten into `~/.claude/skills/` currently leaves only `utils/` (verified, with fix options) |
