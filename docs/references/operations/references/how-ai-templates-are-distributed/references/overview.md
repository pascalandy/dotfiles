...
2. **Source-side category structure that must flatten before fan-out.** The source tree under `dot_config/ai_templates/skills/` is organized into eight workflow-arc buckets: `pa-sdlc/`, `devtools/`, `think/`, `knowledge/`, `web/`, `distill/`, `diagram/`, `media/`. Agents do not load skills from nested category subtrees — they expect a flat `skills/<name>/` layout. The fan-out has to flatten the eight subtrees before any agent home can use them.
...
`fct_compile_assets` at `.chezmoiscripts/run_after_backup.sh:75-104` takes the rendered `commands/` and `skills/` subpaths and produces flat compile directories. For skills, it loops over the eight workflow-arc subtrees (`pa-sdlc`, `devtools`, `think`, `knowledge`, `web`, `distill`, `diagram`, `media`) and uses additive `cp -r` to merge their contents into one directory. For commands (which are already flat in the source), it falls into a flat branch that just copies everything across.
...
- **The source-tree category split (`pa-sdlc/`, `devtools/`, `think/`, `knowledge/`, `web/`, `distill/`, `diagram/`, `media/`) is not visible to any agent.** Skills from all eight buckets land as flat peers under every agent's `skills/` directory. This means a new skill in `devtools/foo/` cannot have the same name as a skill in `knowledge/foo/` — the compile step's `cp -r` silently picks whichever bucket runs last. See [claude-code-flattening.md](claude-code-flattening.md).
