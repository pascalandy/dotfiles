...
- `Product Rules / Constraints`: Limits, rules, guardrails. Every rule must have concrete examples. Apply BDD Principle 1 from `bdd_principles.md`.
- `Edge Cases / Failure Handling`: What breaks and what happens when it does.
- `Acceptance Criteria`: Observable conditions that prove the FEAT works. Each criterion must use concrete scenarios with specific data. Apply BDD Principle 1 and Principle 4.
